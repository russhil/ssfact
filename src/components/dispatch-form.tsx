...
export function DispatchForm({ jobs, defaultArrangedBy = "" }: { jobs: DispatchJob[]; defaultArrangedBy?: string }) {
  ...
  return (
    <div className="rounded-card border border-border bg-surface p-5">
      <h3 className="mb-4 flex items-center gap-2 t-body font-bold">
        <Truck size={15} className="text-primary" /> Log Dispatch
      </h3>

      <label className="mb-1.5 block t-xs font-semibold text-t1">Job Card</label>
      <input
        value={q}
        onChange={(e) => { setQ(e.target.value); setJobId(null); setData(null); }}
        onKeyDown={onKeyDown}
        placeholder="Search SI or item with open balance…"
        className="mb-2 w-full rounded-lg border border-border px-3 py-2.5 t-body outline-none focus:border-primary focus:ring-2 focus:ring-accent/15"
      />
        ...
        <div className="mb-3 max-h-52 overflow-auto rounded-lg border border-border">
          {matches.map((m, i) => (
            <button
              key={m.id}
              onClick={() => pick(m)}
              className={cn("flex w-full items-center justify-between border-b border-hairline px-3 py-2 text-left t-sm last:border-0 hover:bg-primary-soft", i === activeIndex && "bg-primary-soft")}
            >
              <span><b className="text-primary-ink">{m.siNo}</b> <span className="ml-1 text-t2">{m.item}</span> <span className="text-faint">· {m.vendor}</span></span>
      ...
      {job && (
        <div className="mb-1 rounded-lg bg-primary-soft px-3 py-2 t-sm text-primary-ink">
