    ...
    <div className="mt-3 rounded-xl border border-border bg-slate-50/40 p-3.5">
      ...
      <div className="mb-2 rounded-md border border-dashed border-border bg-white/60 p-2">
        <div className="mb-1 flex flex-wrap items-center gap-1.5 text-[11px]">
          <span className="text-[9px] font-semibold uppercase tracking-wide text-faint">This lay&apos;s size ratio</span>
          ...
          <button type="button" onClick={applyRatio} className="rounded-md border border-border bg-white px-2 py-1 font-semibold text-slate-600 hover:bg-slate-50">apply ratio</button>
        </div>
      ...
      <div className="mt-2 grid grid-cols-3 gap-1.5 sm:grid-cols-8">
        <input type="number" step="0.001" value={avg} onChange={(e) => setAvg(e.target.value)} placeholder="avg m/pc (est)" className={`${inp} bg-slate-50 text-faint`} />
        <input type="number" value={rolls} onChange={(e) => setRolls(e.target.value)} placeholder="rolls" className={`${inp} bg-slate-50 text-faint`} />
        <input type="number" step="0.01" value={balance} onChange={(e) => setBalance(e.target.value)} placeholder="balance" className={`${inp} bg-slate-50 text-faint`} />
        ...
        <div className={`rounded-md border border-border bg-slate-50 px-1.5 py-1 text-[11px] font-semibold tnum ${extra != null && extra < 0 ? "text-rose-600" : "text-slate-600"}`} title="Extra = issued − used">
