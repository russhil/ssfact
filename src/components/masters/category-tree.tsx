        ...
        {tree.map((h) => (
          <div key={h.id} className={`rounded-lg border border-border p-3 ${h.active ? "" : "opacity-50"}`}>
            <div className="flex items-center justify-between">
              <input defaultValue={h.label} onBlur={(e) => e.target.value.trim() && e.target.value !== h.label && run(() => updateLookup({ id: h.id, label: e.target.value }))} className="rounded-md border border-transparent px-2 py-1 t-body font-bold outline-none hover:border-border focus:border-primary" />
              <div className="flex items-center gap-2">
                <button onClick={() => setSubFor(subFor === h.id ? null : h.id)} className="inline-flex items-center gap-1 t-xs font-semibold text-primary-ink hover:underline"><CornerDownRight size={12} /> add sub</button>
            ...
            {subFor === h.id && (
              <div className="mt-2 flex gap-2 pl-5">
                <input autoFocus value={sub} onChange={(e) => setSub(e.target.value)} onKeyDown={(e) => e.key === "Enter" && sub.trim() && run(async () => { await createLookup({ kind: "SUB_CATEGORY", label: sub, parentId: h.id }); setSub(""); })} placeholder={`Sub-category under ${h.label}…`} className="w-56 rounded-lg border border-border px-2.5 py-1.5 t-sm outline-none focus:border-primary" />
                <button onClick={() => sub.trim() && run(async () => { await createLookup({ kind: "SUB_CATEGORY", label: sub, parentId: h.id }); setSub(""); })} disabled={busy} className="rounded-lg bg-primary px-2.5 t-sm font-semibold text-accent-on">Add</button>
              </div>
            )}
            {h.children.length > 0 && (
                ...
                {h.children.map((c) => (
                  <div key={c.id} className={`flex items-center justify-between ${c.active ? "" : "opacity-50"}`}>
                    <div className="flex items-center gap-1.5 text-t1">
                      ...
                      <input defaultValue={c.label} onBlur={(e) => e.target.value.trim() && e.target.value !== c.label && run(() => updateLookup({ id: c.id, label: e.target.value }))} className="rounded-md border border-transparent px-1.5 py-0.5 t-sm outline-none hover:border-border focus:border-primary" />
                    </div>
                    <div className="flex items-center gap-2">
                      <select value={c.parentId ?? ""} onChange={(e) => run(() => updateLookup({ id: c.id, parentId: e.target.value ? +e.target.value : null }))} className="rounded-md border border-border px-1.5 py-0.5 t-xs outline-none">
                        {tree.map((hh) => <option key={hh.id} value={hh.id}>{hh.label}</option>)}
                      </select>
                      <button onClick={() => run(() => deactivateLookup({ id: c.id, active: !c.active }))} disabled={busy} className="t-xs text-faint hover:text-danger">{c.active ? "off" : "on"}</button>
                    </div>
                  </div>
                ))}
              </div>
