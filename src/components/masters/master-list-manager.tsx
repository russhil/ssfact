...
export function MasterListManager({ kind, rows, hint }: { kind: string; rows: LookupRow[]; hint?: string }) {
  ...
  return (
    <div>
      {hint && <p className="mb-2 t-sm text-muted">{hint}</p>}
      <div className="mb-3 flex gap-2">
        <input value={draft} onChange={(e) => setDraft(e.target.value)} onKeyDown={(e) => e.key === "Enter" && add()} placeholder="Add a value…" className="w-64 rounded-lg border border-border px-3 py-2 t-body outline-none focus:border-primary" />
