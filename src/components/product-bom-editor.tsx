    ...
    <Card className="mt-3.5 p-5">
      ...
      <div className="mt-3 flex flex-wrap items-end gap-1.5 border-t border-dashed border-border pt-3">
        <label className="flex flex-col gap-1">
          <span className="t-micro uppercase tracking-wide text-faint">Trim (master)</span>
        ...
        <label className="flex flex-col gap-1">
          <span className="t-micro uppercase tracking-wide text-faint">Per pc</span>
        ...
        <button onClick={addLine} disabled={busy || (!nMaterial.trim() && !nTrimId)} className="inline-flex items-center gap-1 rounded-md bg-primary px-3 py-1.5 t-sm font-semibold text-accent-on hover:opacity-90 disabled:opacity-40"><Plus size={13} /> Add</button>
      </div>
    </Card>
  );
}
